...
public class ColorPicker {
    ...
    {
        float deltaR = Color.red(c2) - Color.red(c1);
        float deltaG = Color.green(c2) - Color.green(c1);
        float deltaB = Color.blue(c2) - Color.blue(c1);

        for (int j = y; j<(y+h); j++) {
            int c = this.g.color( 
            	Color.red(c1)+(j-y)*(deltaR/h), 
            	Color.green(c1)+(j-y)*(deltaG/h), 
            	Color.blue(c1)+(j-y)*(deltaB/h) 
            );
            cpImage.set(x, j, c);
        }
    }
    ...
    public void render (boolean mousePressed, int mouseX, int mouseY)
    {
        ...
        if (mousePressed && Geometry.inBound(x, y, w, h, mouseX, mouseY)) {
